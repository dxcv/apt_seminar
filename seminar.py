...
# Create labels
labels      = ['MaxSR-(0,1)', 'gMinVar-(0,1)', '1/N', 'SAA', 'TAA(BL '+MeanModel1+')', 'TAA(BL '+MeanMOdel2+')']

# Plot performance
tdates2     = [datetime.datetime.strptime(str(x)[0:8], '%Y%m%d') for x in tdates]
plt.plot(tdates2, m_value, label=labels[0], color='green')
plt.plot(tdates2, gmv_value, label=labels[1], color='orange')
plt.plot(tdates2, ev_value, label=labels[2], color='blue')
plt.plot(tdates2, saa_value, label=labels[3], color='black')
plt.plot(tdates2, bl1_value, label=labels[4], color='red')
plt.plot(tdates2, bl2_value, label=labels[5], color='magenta')
